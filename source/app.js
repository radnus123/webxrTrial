...
class App {
  ...
  onSelect = () =>{
    ...
    cubeGeo.materials = colors.map(material => new THREE.MeshBasicMaterial(material));
    const cube = new THREE.Mesh(cubeGeo,colors);
    cube.position.copy(this.reticle.position)
    this.scene.add(cube)

    const shadowMesh = this.scene.children.find(c =>c.name === 'shadowMesh');
    if(shadowMesh){
      shadowMesh.position.y = cube.position.y;
    }
  }
}
